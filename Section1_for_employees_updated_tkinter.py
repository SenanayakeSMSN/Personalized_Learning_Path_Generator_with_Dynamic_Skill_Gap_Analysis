...
def extract_skills_with_gemini(cv_text):
    try:
        ...
        prompt = (
            ...
            "Example 3 (no experience):\n"
            "Input: 'Name: Jane Doe\nSkills: Python, Java'\n"
            "Output: {\"structured_cv\":{\"name\":\"Jane Doe\",\"skills\":[\"Python\",\"Java\"],\"experience\":[],\"projects\":[],\"education\":[],\"contact\":\"\",\"experience_duration\":\"0 years 0 months\"},\"skills\":[\"Python\",\"Java\"],\""
            "experience_duration\":\"0 years 0 months\"}\n"
            "Example 4 (multiple jobs, different heading):\n"
            "Input: 'Name: Bob Wilson\nWorking Experience: Developer at Tech Corp, April 2015 - March 2016; Analyst at Data Inc, June 2018 - current'\n"
            "Output: {\"structured_cv\":{\"name\":\"Bob Wilson\",\"skills\":[],\"experience\":[\"Developer at Tech Corp, April 2015 - March 2016\",\"Analyst at Data Inc, June 2018 - current\"],\"projects\":[],\"education\":[],\"contact\":\"\",\"experience_duration\":\"10 years 0 months\"},\"skills\":[],\"experience_duration\":\"10 years 0 months\"}"
            "\n\nCV Text:\n" + cv_text
        )
...
# API key pool (replace with your actual Gemini API keys; use environment variables in production)
API_KEY_POOL = [
...
def configure_gemini_with_retry():
    ...
    max_retries = 3
    retry_delays = [5, 10, 20]  # Seconds to wait for each retry

    for attempt in range(max_retries + 1):  # +1 for initial attempt
        try:
            if CURRENT_API_KEY_INDEX >= len(API_KEY_POOL):
                raise ValueError("All API keys exhausted. Please add more keys or wait for rate limit reset.")
            genai.configure(api_key=API_KEY_POOL[CURRENT_API_KEY_INDEX])
            logging.info(f"Configured Gemini with API key index {CURRENT_API_KEY_INDEX}")
            return True
        except ResourceExhausted as e:
            if attempt < max_retries:
                wait_time = retry_delays[attempt]
                logging.warning(f"Rate limit exceeded. Retrying in {wait_time} seconds with key index {CURRENT_API_KEY_INDEX}...")
                time.sleep(wait_time)
            else:
                logging.warning(f"Retries exhausted for key index {CURRENT_API_KEY_INDEX}. Switching to next key.")
                CURRENT_API_KEY_INDEX += 1
                if CURRENT_API_KEY_INDEX >= len(API_KEY_POOL):
                    logging.error("All API keys exhausted.")
                    return False
        except Exception as e:
            logging.error(f"Error configuring Gemini with key index {CURRENT_API_KEY_INDEX}: {str(e)}")
            CURRENT_API_KEY_INDEX += 1
            if CURRENT_API_KEY_INDEX >= len(API_KEY_POOL):
                logging.error("All API keys exhausted.")
                return False
...
def extract_skills_with_gemini(cv_text):
    try:
        if not configure_gemini_with_retry():
            raise ValueError("Failed to configure Gemini: All API keys exhausted.")
        model = genai.GenerativeModel("gemini-1.5-flash")
        prompt = (
            "You are an expert in CV analysis. Perform the following steps on the provided CV text:\n"
            "1. Parse the CV text into a structured JSON object with the following fields (include only fields that can be identified, leave empty if not found):\n"
            ...
            "Example 3 (no experience):\n"
            "Input: 'Name: Jane Doe\nSkills: Python, Java'\n"
            "Output: {\"structured_cv\":{\"name\":\"Jane Doe\",\"skills\":[\"Python\",\"Java\"],\"experience\":[],\"projects\":[],\"education\":[],\"contact\":\"\",\"experience_duration\":\"0 years 0 months\"},\"skills\":[\"Python\",\"Java\"],\"experience_duration\":\"0 years 0 months\"}\n"
            "Example 4 (multiple jobs, different heading):\n"
            "Input: 'Name: Bob Wilson\nWorking Experience: Developer at Tech Corp, April 2015 - March 2016; Analyst at Data Inc, June 2018 - current'\n"
            "Output: {\"structured_cv\":{\"name\":\"Bob Wilson\",\"skills\":[],\"experience\":[\"Developer at Tech Corp, April 2015 - March 2016\",\"Analyst at Data Inc, June 2018 - current\"],\"projects\":[],\"education\":[],\"contact\":\"\",\"experience_duration\":\"10 years 0 months\"},\"skills\":[],\"experience_duration\":\"10 years 0 months\"}"
            "\n\nCV Text:\n" + cv_text
        ...
        for attempt in range(max_retries):
            try:
                ...
                try:
                    ...
                    if not isinstance(skills, list):
                        logging.error("Gemini skills field is not a JSON array")
                        return [], structured_cv, "0 years 0 months"
                    if not isinstance(experience_duration, str):
                        logging.error("Gemini experience_duration field is not a string")
                        return [], structured_cv, "0 years 0 months"
                    logging.info(f"Structured CV: {json.dumps(structured_cv, indent=2)}")
                ...
                if attempt < max_retries - 1:
                    wait_time = retry_delays[attempt]
                    logging.warning(f"Rate limit exceeded in skill extraction. Retrying in {wait_time} seconds...")
                    time.sleep(wait_time)
                else:
                    logging.warning(f"Retries exhausted for skill extraction. Switching API key.")
                    CURRENT_API_KEY_INDEX += 1
                    if not configure_gemini_with_retry():
                        raise ValueError("Failed to analyze CV: All API keys exhausted.")
                    model = genai.GenerativeModel("gemini-1.5-flash")
            except Exception as e:
                logging.error(f"Error in Gemini skill extraction: {str(e)}")
                return [], {}, "0 years 0 months"
        raise ValueError("Failed to analyze CV after retries and key switches.")
    except Exception as e:
        logging.error(f"Error in Gemini skill extraction: {str(e)}")
        return [], {}, "0 years 0 months"
    ...
    try:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    wait_time = retry_delays[attempt]
                    logging.warning(f"Rate limit exceeded in question answering. Retrying in {wait_time} seconds...")
                    time.sleep(wait_time)
                else:
                    logging.warning(f"Retries exhausted for question answering. Switching API key.")
                    CURRENT_API_KEY_INDEX += 1
                    if not configure_gemini_with_retry():
                        raise ValueError("Failed to answer question: All API keys exhausted.")
                    model = genai.GenerativeModel("gemini-1.5-flash")
            except Exception as e:
                logging.error(f"Error answering question with Gemini: {str(e)}")
                return f"Error answering question: {str(e)}"
        raise ValueError("Failed to answer question after retries and key switches.")
    except Exception as e:
        logging.error(f"Error answering question with Gemini: {str(e)}")
        return f"Error answering question: {str(e)}"
...
class SkillGapAnalyzerApp:
    ...
    def ask_question(self):
        ...
        try:
            answer = answer_cv_question(
                ...
                question,
                self.skill_gap_result,
                self.skill_gap_result.get("experience_duration", "0 years 0 months")
            )
            self.result_text.delete(1.0, tk.END)
            self.result_text.insert(tk.END, f"Question: {question}\nAnswer: {answer}\n")
            self.question_entry.delete(0, tk.END)  # Clear question input
    ...
    def generate_learning_path(self):
        if not self.cv_text or not self.structured_cv or not self.skill_gap_result:
            messagebox.showerror("Error", "Please analyze a CV first.")
            return
        question = f"Generate a learning path to acquire the missing skills ({', '.join(self.skill_gap_result.get('missing_skills', []))}) for the job role '{self.skill_gap_result.get('job_role', '')}', considering the candidate has {self.skill_gap_result.get('experience_duration', '0 years 0 months')} of work experience. Include specific courses, resources, and a timeline."
        self.result_text.insert(tk.END, f"\nGenerating Learning Path...\n")
        self.root.update()
        try:
            learning_path = answer_cv_question(
